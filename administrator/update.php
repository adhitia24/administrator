		...
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'update'){
				...
				if(mysqli_num_rows($cek) == 0){
					...
					$update = mysqli_query($koneksi, "UPDATE FROM pemesanan WHERE kode_pengiriman='$kode_pengiriman'");
					if($update){
				...
				if(mysqli_num_rows($sql) == 0){
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						echo '
							...
							<td>
								<form name="formcari" method="post" action="edit.php">
  <input type="submit" name="submit" id="submit" class="button" value="TRACKING NOW">		
 </form>
							</td>
						</tr>
						';
